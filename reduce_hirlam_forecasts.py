...
grbs = pygrib.open("/data/grib/pressure-numerical-hirlam74-forecast-WindUMS-20210524T000000Z.grb2")
#for grb in grbs[:4]:
#    print(grb)
...
for grb in grbs:
	...
	print(grb.level)
	if grb.validDate.date() == date_valid.date() and grb.parameterName == 'u-component of wind' and grb.level == 1000:
		uwind.append(grb.values)
		datetimes.append(grb.validDate)
uwind = np.array(uwind)


grbs = pygrib.open("/data/grib/pressure-numerical-hirlam74-forecast-WindVMS-20210524T000000Z.grb2")
#for grb in grbs[:4]:
#    print(grb)
...
for grb in grbs:
	...
	print(grb.parameterName)
	print(grb.level)
	if grb.validDate.date() == date_valid.date() and grb.parameterName == 'v-component of wind' and grb.level == 1000:
...
with imageio.get_writer('/data/out/%s.gif' % 1000, mode='I') as writer:
		idx = 0
		for d in datetimes:
				...
				filename = '/data/out/%s-%s.png' % (1000, d.isoformat(timespec="seconds"))
				cb=plt.colorbar()
